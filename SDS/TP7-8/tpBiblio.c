    ...
    do {
        ...
        switch(chx) {
            ...
            case 4:
                ...
                break;
            case 5:
                lireChaine("Code du livre: ", code, K_MaxCode);
                reponse = supprimerLivre(&B, code);
                if (reponse == 0)
            ...
            case 6:
                lireChaine("Code du livre: ", code, K_MaxCode);
                lireChaine("Nom de l'emprunteur: ", auteur, K_MaxEmp);
                reponse = emprunterLivre(&B, code, auteur);
                if (reponse == 0)
                    printf("Le livre %s a bien ete emprunte par %s.", code, auteur);
                else
                    printf("Le livre n'a pas été emprunté.");
                break;
            case 7:
                lireChaine("Code du livre: ", code, K_MaxCode);
                retournerLivre(&B, code);
                printf("Le livre a bien ete rendu.");

                break;
            case 8:
                trierParTitre(&B);
                printf("La bibliotheque a ete triee par titre.");
                break;
            case 9:
                trierParAuteur(&B);
                printf("La bibliotheque a ete triee par auteur.");
                break;
            case 10:
                trierParAnnee(&B);
                printf("La bibliotheque a ete triee par annee.");
                break;
            }
    } while(chx!=0);
    sauvegarde(&B);
