...
function listerUtilisateurs($classe = "both")
{
	...
	//die($SQL);	// réalise un affichage ET interrompt l'exécution php 
	
	$rs = SQLSelect($SQL);  // renvoie un objet php "recordset" 
	$tab = parcoursRs($rs); // "tableau de tableaux associatifs"
	return $tab; 
	
	// possible aussi return parcoursRs(SQLSelect($SQL)); 
...
function interdireUtilisateur($idUser)
{
	...
	// injection SQL 
	// contre-mesures 
	// 1) encadrer les entrees user par des apostrophes
	// Si idUser = 1'; drop table users;' DANGER ! 
	// 2) banaliser les caractères spéciaux SQL, notamment les apostrophes
	// => objectif de la fonction valider qui appelle proteger 
	
	// Alternative : utiliser des requetes préparées 
	// comme un printf("format avec des %s",var1, var2) 
